...
def handle_index(chunk_size: int, chunk_overlap: int) -> tuple[str, str]:
    """
    ...
    Returns:
        Tuple of (status message, updated index status).
    """
    try:
        # Show what directory we're looking in for debugging
        logger.info("Indexing documents from: %s", DOCUMENTS_DIR)

        chunks, errors = load_and_split(
            directory=DOCUMENTS_DIR,
            chunk_size=int(chunk_size),
            chunk_overlap=int(chunk_overlap),
        )

        # Report any file-level errors
        error_msg = ""
        if errors:
        ...
        if not chunks:
            return (
                f"⚠️ No documents could be loaded.\n\n"
        ...
        return f"❌ Indexing failed: {exc}", _get_status()
...
def build_tab() -> gr.TabItem:
    ...
    with gr.TabItem("📁 Documents", id="tab_documents") as tab:
        ...
        index_btn.click(
            ...
            outputs=[status_msg, index_status],
        )
