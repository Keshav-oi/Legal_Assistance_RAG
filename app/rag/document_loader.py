...
def _load_single_file(file_path: Path) -> list[Document]:
    ...
    Raises:
        Exception: If the file loader fails — propagated to caller for reporting.
    """
...
def load_documents(directory: Path | None = None) -> tuple[list[Document], list[str]]:
    """
    ...
    Returns:
        Tuple of (loaded documents, list of error messages).
    """
    ...
    if not files_found:
        ...
        return [], [f"No supported files in {directory}"]

    logger.info("Found %d file(s) to load in %s", len(files_found), directory)
    ...
    logger.info("Total documents loaded: %d (errors: %d)", len(all_docs), len(errors))
...
def load_and_split(
    ...
    Returns:
        Tuple of (chunked Document objects, list of error messages).
    """
    docs, errors = load_documents(directory)
